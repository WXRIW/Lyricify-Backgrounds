...
namespace Lyricify.Backgrounds.AppleMusicInspired.Rendering
{
    ...
    {
        public AppleMusicPinchVertex(Vector2 from, Vector2 to, Vector2 textureCoordinate)
        {
            From = from;
            To = to;
    ...
    public static class AppleMusicInspiredMesh
    {
        ...
        public const int MaximumControlPointCount = 16;
        public const int MaximumSubdivisionLevels = 4;
        internal const int DefaultPortraitControlPointCount = 6;
        internal const int DefaultPortraitSubdivisionLevels = 2;
        internal const int DefaultLandscapeControlPointCount = 9;
        internal const int DefaultLandscapeSubdivisionLevels = 2;

        // Five selectable slots map to four distinct portrait meshes.
        public static int SelectPreset()
        {
        ...
        {
            return Math.Clamp(presetSlot, 0, PresetSlotCount - 1) switch
            {
                0 => 0,
                1 => 1,
                2 or 3 => 2,
        ...
        // Landscape mode provides five distinct meshes.
        public static int SelectLandscapePreset()
        {
            return SelectPresetSlot();
        }

        public static (AppleMusicPinchVertex[] Vertices, ushort[] Indices) Create(
            int presetIndex,
            bool isVerticalLayout = true,
            int? controlPointCount = null,
            int? subdivisionLevels = null)
        {
            ...
            int sourceControlPointCount = isVerticalLayout
                ? DefaultPortraitControlPointCount
                : DefaultLandscapeControlPointCount;
            int defaultSubdivisionLevels = isVerticalLayout
                ? DefaultPortraitSubdivisionLevels
                : DefaultLandscapeSubdivisionLevels;
            int effectiveControlPointCount = Math.Clamp(
                controlPointCount is null or < 0
                    ? sourceControlPointCount
                    : controlPointCount.Value,
                MinimumControlPointCount,
                MaximumControlPointCount);
            int effectiveSubdivisionLevels = Math.Clamp(
                subdivisionLevels is null or < 0
                    ? defaultSubdivisionLevels
                    : subdivisionLevels.Value,
                0,
                MaximumSubdivisionLevels);
            Vector2[,] from = ToGrid(preset.From, sourceControlPointCount);
            Vector2[,] to = ToGrid(preset.To, sourceControlPointCount);
            if (effectiveControlPointCount != sourceControlPointCount)
            {
                from = ResampleGrid(from, effectiveControlPointCount);
                to = ResampleGrid(to, effectiveControlPointCount);
            }

            for (int level = 0; level < effectiveSubdivisionLevels; level++)
            {
                from = Subdivide(from);
                to = Subdivide(to);
            }
            ...
            var vertices = new AppleMusicPinchVertex[rows * columns];
            for (int row = 0; row < rows; row++)
            {
                ...
                for (int column = 0; column < columns; column++)
                {
                    ...
                    vertices[row * columns + column] = new AppleMusicPinchVertex(
        ...
        private static Vector2[,] ResampleGrid(
            Vector2[,] source,
            int controlPointCount)
        {
            ...
            for (int row = 0; row < controlPointCount; row++)
            {
                float sourceY = row * (sourceRows - 1f) / denominator;
                int firstRow = Math.Min((int)sourceY, sourceRows - 1);
                int secondRow = Math.Min(firstRow + 1, sourceRows - 1);
                float rowMix = sourceY - firstRow;
                for (int column = 0; column < controlPointCount; column++)
                {
                    float sourceX = column * (sourceColumns - 1f) / denominator;
                    int firstColumn = Math.Min((int)sourceX, sourceColumns - 1);
                    int secondColumn = Math.Min(firstColumn + 1, sourceColumns - 1);
                    float columnMix = sourceX - firstColumn;
                    Vector2 first = Vector2.Lerp(
                        source[firstRow, firstColumn],
                        source[firstRow, secondColumn],
                        columnMix);
                    Vector2 second = Vector2.Lerp(
                        source[secondRow, firstColumn],
                        source[secondRow, secondColumn],
                        columnMix);
                    result[row, column] = Vector2.Lerp(first, second, rowMix);
                }
            }
            return result;
        }
        ...
        // All landscape maps start from the same regular 9x9 lattice.
